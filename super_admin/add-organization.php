                  ...
                  <div class="card-body">
                    ...
                    <form class="forms-sample" method="post" action="inputConfig.php">
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" name="org_name" value="<?=$org_name?>" class="form-control" required='true'>
                      </div>                      
                      <div class="form-group">
                        <label for="">Established Date: </label>
                        <input type="date" name="org_date" value="<?=$org_create?>" class="form-control" required='true'>
                      </div>
                      <div class="form-group">
                        <label for="">Barangay: </label>
                        <select  name="address" class="form-control" required='true' value="<?=$brgy?>">
                          <?php
                            if(empty($brgy)){
                                echo "<option selected disabled>Choose Barangay</option>";
                            }
                            $brgy_list = new fetch();
                            ...
                            if($res->rowCount()>0){
                              while ($row = $res->fetch()) {
                                if($row['brgy_name'] == $brgy ){
                                  echo "<option selected>".$row['brgy_name']."</option>";
                                }else{
