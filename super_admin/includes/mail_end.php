...
foreach ($res as $row){
    $value_id_num = $row['org_mem_name_id'];<?php
require_once '../PHPMailer/PHPMailerAutoload.php';
...
?>
    ...
    if($res_account->rowCount()){
        ...
        $mail->clearAddresses();
        $mail->setFrom('jpaulgalicha01@gmail.com','HWFMS - Hinigaran Womens Fedration Managament System');
        $mail->addAddress($res_account_fetch['acc_email']);
        
        //The subject of the message.
        $mail->Subject = 'Registration For Admin Barangay (Closed)';
        $message = '
